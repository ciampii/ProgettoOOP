...
public class ApiDate {
	...
	private Vector<String> dataPartita= new Vector<String>();
...
try {
	...
	for (int i = 0; i < dataPartita.size(); i++) {
		System.out.println( dataPartita.get(i)+" " + squadraCasa.get(i) + "-" + squadraOspite.get(i) +" "+ punteggioCasa.get(i)+"-"+punteggioOspite.get(i));
		}
	...
	public String readVectors() {
		String lunga ="";
		for (int i = 0; i < dataPartita.size(); i++) {
			lunga.concat(dataPartita.get(i)+" " +punteggioCasa.get(i) + " " + punteggioOspite.get(i)+" "+  punteggioCasa.get(i)+"-"+punteggioOspite.get(i) );
		}
		return lunga;
}
}
